...
void main(void)
{
    ...
    /* Observa que las variables *a3, *a4, *a5 se declaran como apuntadores de tipo estructura alumno. a6 es una variable de tipo escructura alumno. */
    ...
    /* Nota que al apuntador a4 es necesario asignarle una direccion de memoria. para tener acceso a los campos de un tipo apuntador de tipo estructura, utiliza uno de los dos formatos siguientes:

    apuntador>campo
    o bien
    (*apuntador).campo

    en la lectura de los campos de la variable a4 se utilizan como ejemplos ambos formatos. */
    ...
    lectura(a5);   /* En este caso se pasa el apuntaor de tipo estructura alumno a5 a la funcion lectura. */

    lectura(&a6); /* En este caso se para la cariable de tipo estructura alumno a6, a la funcion Lectura. Observa que en este caso debemos utilizar el operador de direccion para preceder a la variable. */
    ...
    /* Observa la forma de escribir los campos de los apuntadores de tipo estructura. */
        ...
        printf("\nDatos del alumno 4\n");
        printf("%d\t%s\t%.2f\t%s", a4->matricula, a4->nombre, a4->carrera, a4->promedio, a4->direccion);

        printf("\nDatos del alumno 5\n")
        printf("%d\t%s\t%.2f\t%s", a5->matricula, a5->nombre, a5->carrera, a5->promedio, a5->direccion);

        printf("\nDatos del alumno 6\n");

    /*Observa la fomra de escribir los campos de la variable tipo estructura. */
...
/* Esta funcion permite leer los campos de un apuntador de tipo estructura alumno. */
{
    ...
    scanf("%d", &(*a).matricula);
    fflush(stdin);
    printf("Ingrese el nombre dek alumno: ");
    gets(a->nombre);
    fflush(stdin);
